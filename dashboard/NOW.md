...
- MCP privé opérationnel pour Dolibarr, Paheko, Nextcloud, Pretix, PeerTube et IMAP. Les boîtes professionnelles `contact@alpinechain.xyz` et `cyrille@alpinechain.xyz` sont accessibles séparément en lecture seule, sur `INBOX`, et consultées à la demande au même rythme. Décision validée : GPT reste l’interface de conversation, GitHub le cockpit et Codex accède au MCP par tunnel SSH ; aucune exposition publique n’est requise.
